...
(function (window, $, undefined) {
    ...
    $.html5imageupload = function html5imageupload(options, element) {
        ...
        this.imageExtensions	= {png: 'png', bmp: 'bmp', jpg: ['jpg','jpeg']};
        this.imageMimes = {
            png: 'image/png',
            bmp: 'image/bmp',
            jpg: 'image/jpeg',
    ...
    $.html5imageupload.prototype = {
        _init: function () {
            ...
            if (!empty(options.image) && options.editstart == false) {
                ...
                $(element).data('name', options.image).append($('<img />').addClass('preview').attr('id', 'user_avatar').attr('src', options.image));
        ...
        readImage: function (image, src, name, mimeType) {
            ...
            img.onload = function (tmp) {
                //console.log(tmp);
                var imgElement = $('<img id="image_upload" src="' + src + '" name="' + name + '" />');
        ...
        reset: function () {
            ...
            $(element).removeClass('loading done').children().show().not('input[type=file],img').remove();
            $(input).wrap('<form>').parent('form').trigger('reset');
            $(input).unwrap();
